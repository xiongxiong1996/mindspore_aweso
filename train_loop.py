...
def train_loop(model, dataset, loss_fn, optimizer, args):
    ...
    # relu = ops.ReLU()
    #
    # def get_part_score(logits, targets):
    #     log_softmax = nn.LogSoftmax()
    #     temp = log_softmax(logits)  # (bs*topn,calss_n)
    #     score = zeros((args.batch_size * args.topk, 1), mstype.float32)
    #     for i in range(logits.shape[0]):
    #         list_item = -temp[i][targets[i]]
    #         score[i] = list_item
    #     score = opReshape(score, (args.batch_size, args.topk))
    #     return score
    #
    # def get_ranking_loss(score, part_score):
    #     loss = ms.Tensor(0.)
    #     batch_size = score.shape[0]
    #     for i in range(args.topk):
    #         part_score_i = opExpand_dims(part_score[:, i], 1)
    #         targets_p = (part_score > part_score_i)  # .TYPE FLOAT UNQUIZE1
